...
const groupId = process.env.DEFAULT_GROUP_ID || '01HYZW0Z01R04Z6H9AJW2AG0DN';
...
async function saveNewPrefixes(prefixes: string[]): Promise<void> {
  ...
  const insertData: QueueEntry[] = newPrefixes.map((prefix) => ({
    id: ulid(),
    name: prefix,
    host: redisHost,
    port: redisPort,
  ...
  const insertDataArray: string[][] = insertData.map((entry) => Object.values(entry));

  const insertQuery = 'INSERT INTO queue (id, name, host, port, health_value, group_id) VALUES ?';
  return new Promise((resolve, reject) => {
