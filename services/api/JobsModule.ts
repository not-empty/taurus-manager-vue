...
export class JobsModule extends Module {
  ...
  public async retryJob (
    queueId: string,
    payload: payloadCreateJobRetryJob
  ): Promise<boolean> {
    const result = await this.api.$post<boolean>(
  ...
  public async getJobPaginate (
    queueId: string,
    page: number,
    size: number,
    state: string
  ): Promise<ApiResponseJobsPaginated<IJob[]>> {
    const result = await this.api.$get<ApiResponseJobsPaginated<IJob[]>>(
      `${this.path}/${queueId}/job?state=${state}&page=${page}&size=${size}`
    );
  ...
  public async exportJob (queueId: string, jobId: string): Promise<IJob> {
    const result = await this.api.$get<IJob>(
      `${this.path}/${queueId}/job/${jobId}/export`
    );
  ...
  public async deleteJob (
    queueId: string,
    jobsIds: Omit<payloadCreateJobRetryJob, 'state'>
  ): Promise<boolean> {
